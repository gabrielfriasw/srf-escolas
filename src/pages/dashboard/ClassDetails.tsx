...
export const ClassDetails: React.FC = () => {
  ...
  // Verifica se o usuário tem permissão para gerenciar a turma
  const canManageClass = user?.role === 'COORDINATOR' || classData?.owner_id === user?.id;
  ...
  useEffect(() => {
    if (classData) {
      ...
      if (classData.name === 'Peter Alert') {
        setShowPeterAlert(true);
      }
    }
  }, [classData]);
