...
export const NewClass: React.FC = () => {
  ...
  // Check and fix user profile on component mount
  useEffect(() => {
      ...
      try {
        ...
        if (!profile) {
          ...
          if (createError) {
            ...
            setError('Erro ao criar perfil de usuário. Por favor, faça logout e login novamente.');
          }
        }
      } catch (err) {
        console.error('Erro ao verificar/criar perfil:', err);
        setError('Erro ao verificar perfil de usuário. Por favor, faça logout e login novamente.');
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.grade.trim() || !formData.pedagogist_phone.trim() || !formData.shift) {
    ...
    try {
      await addClass({
        ...
        pedagogist_phone: formData.pedagogist_phone.trim(),
