...
export const AddStudentForm: React.FC<AddStudentFormProps> = ({ classId, onAddStudent }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await studentsService.addStudent(classId, {
        name: newStudent.name.trim(),
        number,
      });
      
      setNewStudent({ name: '', number: '' });
      await fetchClasses(); // Força atualização da lista
      onAddStudent();
    } catch (error: any) {
